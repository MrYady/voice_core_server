...
3. **Analizador Semántico (`semantic_analyzer.py`)**: Módulo POO que se conecta de forma nativa a la infraestructura de Hugging Face para cargar los pesos de **BETO (BERT en Español)** ajustado a medida (*fine-tuned*). Determina en milisegundos si una frase está `COMPLETA` o `INCOMPLETA` para retener o despachar el texto.
4. **Agente Autónomo (`agent_llama.py`)**: Consume las frases completas publicadas por el servidor e interactúa de forma local con **Ollama (Llama 3:8B)** manteniendo el hilo contextual de la conversación.
...
├── agent_llama.py            # Suscriptor / Agente inteligente (llama3:8b)
├── microphone_client.py      # Cliente emisor de audio por micrófono
├── semantic_analyzer.py      # Módulo POO del clasificador sintáctico remoto
